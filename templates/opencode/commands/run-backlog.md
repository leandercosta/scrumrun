...
- `--list` (`-l`) `[filter]`: read `.scrumrun/backlog.md`, `.scrumrun/goals/main/sprint.md`, and `.scrumrun/goals/main/history.md`, then list backlog candidates showing the backlog item, the matching sprint title or focus when discoverable, whether history marks it completed, partial, or blocked, and the exact command to run it, such as `/run-sprint --run Sprint 01`. Do not modify files.
