...
const ARTIFACT_TYPES = Object.freeze({
  feature: { prefix: "FEAT", directory: "features", statuses: ["backlog", "proposed", "active", "completed", "paused", "cancelled"] },
  task: { prefix: "TASK", directory: "tasks", statuses: ["backlog", "proposed", "running", "validating", "learning", "partial", "completed", "failed", "blocked", "cancelled"] },
  sprint: { prefix: "SPRINT", directory: "sprints", statuses: ["proposed", "running", "partial", "completed", "blocked", "cancelled"] },
  run: { prefix: "RUN", directory: "runs", statuses: ["executing", "validating", "learning", "partial", "completed", "failed", "blocked"] },
  review: { prefix: "REV", directory: "reviews", statuses: ["proposed", "running", "passed", "failed", "archived"] },
  knowledge: { prefix: "K", directory: path.join("memory", "knowledge"), statuses: ["candidate", "approved", "rejected", "deprecated", "invalidated"] },
  decision: { prefix: "DEC", directory: path.join("memory", "decisions"), statuses: ["open", "resolved", "deprecated", "invalidated"] },
  insight: { prefix: "INS", directory: path.join("memory", "insights"), statuses: ["candidate", "confirmed", "stale", "deprecated", "invalidated"] },
  dossier: { prefix: "DOS", directory: path.join("memory", "dossiers"), statuses: ["active", "stale", "deprecated", "archived"] }
});
...
function validateArtifact(record, expectedKind = null) {
  ...
  if (record.kind === "run" && !/^TASK-\d{3,}$/.test(record.task || "")) errors.push("run.task must reference TASK-NNN");
  const references = {
    feature: ["FEAT", "feature"],
    sprint: ["SPRINT", "sprint"],
    task: ["TASK", "task"]
  };
  for (const [field, [prefix, label]] of Object.entries(references)) {
    if (record[field] !== undefined && record[field] !== null && !new RegExp(`^${prefix}-\\d{3,}$`).test(record[field])) {
      errors.push(`${record.kind}.${field} must reference ${label.toUpperCase()}-NNN or null`);
    }
  }
  return errors;
}
