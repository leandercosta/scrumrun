...
function migrationPlan(projectRoot) {
  ...
  for (const entry of inventory.files) {
    if (/(^|\/)vault(?:\.local)?\.md$/i.test(entry.path) || entry.size > 1024 * 1024 || !/\.(?:md|json|txt)$/i.test(entry.path)) continue;
  ...
  function migrateGuardrails() {
    ...
    if (!golden && /^# ScrumRun Project Guardrails\b/m.test(current) && /^## GR-\d{3,}\b/m.test(current)) return;
  ...
  generated.set("method.json", `${JSON.stringify({ method: METHOD_VERSION, layout: "v2", migrated_from: "1.x", migration: MIGRATION_NAME }, null, 2)}\n`);
...
function validateStage(stage, plan) {
  ...
  for (const kind of Object.keys(ARTIFACT_TYPES)) {
    for (const artifact of repository.list(kind)) {
      if (artifact.record && !artifact.errors.length) records.set(artifact.record.id, artifact.record);
    }
  }
...
function applyMigration(projectRoot, { failurePoint = null } = {}) {
