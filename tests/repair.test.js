...
test("repair converts legacy Metadata artifacts in canonical directories without losing the body", () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(scrum, "tasks", "TASK-148.md"), legacy);

    const result = repair(scrum, { apply: true });
    assert.ok(result.applied.applied.some((entry) => entry.file === "tasks/TASK-148.md"));
    const converted = fs.readFileSync(path.join(scrum, "tasks", "TASK-148.md"), "utf8");
    assert.match(converted, /^id: TASK-148$/m);
    assert.match(converted, /^kind: task$/m);
    assert.match(converted, /^status: completed$/m);
    assert.match(converted, /^type: feature$/m);
    assert.match(converted, /^feature: FEAT-007$/m);
    assert.match(converted, /Keep this body byte-for-byte\./);
    assert.ok(fs.existsSync(path.join(scrum, ".migration-backup", "repair", "tasks", "TASK-148.md")));
  } finally {
