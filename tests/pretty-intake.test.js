...
test("renderIntakePlain includes materialization when the plan proposes more than Task + Run", () => {
  const plain = renderIntakePlain(fakePlan({
    classification: { type: "feature", reason: "long-lived initiative" },
    proposal: { materialization: { summary: "Feature + Task + Run" } }
  }));
  assert.match(plain, /^Materialization: Feature \+ Task \+ Run$/m);
});
